...
namespace VoxelSystem {
	...
	public class MyGPUVoxelizer {
        ...
        public static GPUVoxelData Voxelize(ComputeShader voxelizer, MeshFilter[] meshFilters, int resolution = 32, bool volume = true, bool pow2 = false) {
	        Bounds b = meshFilters[0].mesh.bounds;
	        b.center += meshFilters[0].transform.localPosition;
	        foreach (var meshFilter in meshFilters)
	        {
		        meshFilter.mesh.RecalculateBounds();
		        Bounds b2 = meshFilter.mesh.bounds;
		        b2.center += meshFilter.transform.localPosition;
		        b.Encapsulate(b2);
	        }
	        return Voxelize(voxelizer, meshFilters, b, resolution, volume, pow2);
        }
        
        public static GPUVoxelData Voxelize(ComputeShader voxelizer, MeshFilter[] meshFilters, Bounds bounds, int resolution = 32, bool volume = true, bool pow2 = false)
        {
			...
			foreach (var meshFilter in meshFilters)
			{
				...
				var meshVertices = new List<Vector3>(meshFilter.mesh.vertices);
				for (var i = 0; i < meshVertices.Count; i++)
				{
					//meshVertices[i] += meshTransform.localPosition;
					meshVertices[i] = RotateX(meshVertices[i], meshTransform.localEulerAngles.x);
					meshVertices[i] = RotateY(meshVertices[i], meshTransform.localEulerAngles.y);
					meshVertices[i] = RotateZ(meshVertices[i], meshTransform.localEulerAngles.z);
					meshVertices[i] = new Vector3(meshVertices[i].x * meshTransform.localScale.x, 
				...
				var list = meshFilter.mesh.triangles;
		...
		public static GPUVoxelData Voxelize(ComputeShader voxelizer, List<Mesh> meshes, int resolution = 32, bool volume = true, bool pow2 = false) {
			...
			return Voxelize(voxelizer, meshes, b, resolution, volume, pow2);
		}

        public static GPUVoxelData Voxelize(ComputeShader voxelizer, List<Mesh> meshes, Bounds bounds, int resolution = 32, bool volume = true, bool pow2 = false)
        {
	        var vertices = new List<Vector3>();
	        var triangles = new List<int>();
			foreach (var mesh in meshes)
