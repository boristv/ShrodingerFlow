...
public static class HybridBuildingSceneSetup
{
    ...
    private static void AttachHybrid(GameObject go)
    {
        ...
        SetV3(so, "_inletVelocity", new Vector3(0.6f, 0f, 0f)); // горизонталь — несёт дым через здание
        // Вентилируемый режим (рабочий): сквозняк несёт дым через здание + умеренная плавучесть.
        // Чтобы вытяжка стала заметной (вкл/выкл), на лету выключи _openFlowFaces (закрыть грани) —
        // тогда вытяжка станет единственным выходом. Маски перестраиваются в рантайме.
