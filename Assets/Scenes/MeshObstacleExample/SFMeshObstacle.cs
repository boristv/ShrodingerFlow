...
public class SFMeshObstacle : SFBase
{
    ...
    [SerializeField] private Vector3Int offset = new Vector3Int(36, 14, 9);
    
    [Header("Начальное расположение частиц")] 
    [SerializeField] private Vector2 _boxSizeX = new Vector2(0.3f, 0.3f);
    ...
    {
        isObstacle = new bool[vol_res[0], vol_res[1], vol_res[2]];
        var points = MeshConverter.Convert(_obstacleModel, resolution, voxelizer, type);
        
        points = MeshConverter.Rotate(points, _obstacleModel.transform.eulerAngles);
        
        if (MeshConverter.TryConvertToGrid(points, out var isObstacleInside, out var size))
        {
            Debug.Log($"mesh res: x = {size.x} | y = {size.y} | z = {size.z}");
            
            for (int x = 0; x < Mathf.Min(size.x, (vol_res[0] - offset.x)); x++)
            {
                for (int y = 0; y < Mathf.Min(size.y, (vol_res[1] - offset.y)); y++)
                {
                    for (int z = 0; z < Mathf.Min(size.z, (vol_res[2] - offset.z)); z++)
                    {
                        isObstacle[x + offset.x, y + offset.y, z + offset.z] = isObstacleInside[x, y, z];
                    }
                }
            }
            ...
            //return;
            var scale = (2f / 64f);
            for (int x = 0; x < vol_res[0]; x++)
            {
                for (int y = 0; y < vol_res[1]; y++)
                {
                    for (int z = 0; z < vol_res[2]; z++)
                    {
                        if (isObstacle[x, y, z])
                        {
                            var go = Instantiate(_box, scale * new Vector3(x, y, z), Quaternion.identity, _parent);
